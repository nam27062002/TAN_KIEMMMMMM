    ...
    protected override DamageTakenParams GetDamageParams_Skill2_MyTurn(Character character)
    {
        ...
        foreach (var friend in friends)
        {
            ...
            if (roll >= 10)
            {
                ...
                AlkawaDebug.Log(ELogCategory.SKILL, $"[{CharName}] Ném Đá: 1d20 = {roll} >= 10 => có thể cùng tấn công");
                ...
                AlkawaDebug.Log(ELogCategory.SKILL, $"[{CharName}] Ném Đá: damage = {friendBaseDamage} + {friendSkillDamage} = {friendTotalDamage}");
            }
            else
            {
                AlkawaDebug.Log(ELogCategory.SKILL, $"[{CharName}] Ném Đá: 1d20 = {roll} < 10 => Không thể cùng tấn công");
            }
        }
